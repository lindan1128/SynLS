...
def build_model(time_len, fea_num, d_model=16, n_heads=2, encoder_type='dual'):
    ...
    if encoder_type in ['time', 'dual']:
        time_encoded = time_transformer_encoder(x_input, time_emb, d_model=d_model, n_heads=n_heads)
        encoded_features.append(time_encoded)

    if encoder_type in ['pairwise', 'dual']:
        pairwise_encoded = pairwise_transformer_encoder(x_input, time_emb, d_model=d_model, n_heads=n_heads)
...
class DiffusionModel(keras.Model):
    ...
    def check_noise_levels(self):
        ...
        timesteps_to_check = [0, self.timesteps//4, self.timesteps//2, 3*self.timesteps//4, self.timesteps-1]
        ...
        for t in timesteps_to_check:
            ...
            print(f"t={t}: error={tf.reduce_mean(tf.abs(noise - pred_noise)):.4f}")
